internal enum CapacityTable {
    // [version][errorCorrection: L,M,Q,H][mode: numeric, alphanumeric, byte]
    private static let capacities: [[[Int]]] = [
    ...
    static func capacity(version: Int, errorCorrection: QRErrorCorrectionLevel, mode: QREncodingMode) -> Int {
        guard version >= 1 && version <= 40 else { return 0 }

        let modeIndex: Int
        switch mode {
        case .numeric: modeIndex = 0
        case .alphanumeric: modeIndex = 1
        case .byte: modeIndex = 2
        }

        return capacities[version][errorCorrection.tableIndex][modeIndex]
    }
}
